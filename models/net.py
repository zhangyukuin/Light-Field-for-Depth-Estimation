...
class modelfinal(nn.Module):
        def __init__(self):
            ...
            self.conv3 = nn.Conv2d(
                36, 18, kernel_size=3, stride=1, padding=1, bias=False)
            self.paclite = modules.PacJointUpsampleLite(channels=1, guide_channels=3, factor=4)

        def forward(self, x,focal):

            out = self.paclite(x,focal)
